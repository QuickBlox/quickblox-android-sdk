 ...
 */
public class CallActivityTemp extends BaseActivity implements QBRTCClientSessionCallbacks, QBRTCSessionConnectionCallbacks, QBRTCSignalingCallback,
    ...
    private com.quickblox.sample.groupchatwebrtc.definitions.Consts.StartConversationReason startConversationReason;
    private ArrayList<Integer> opponentsIdsList;
    private QBRTCTypes.QBConferenceType conferencType;
    ...
    public static void start(Context context,
                             ...
                             com.quickblox.sample.groupchatwebrtc.definitions.Consts.StartConversationReason startConversationReason){
...
//            addOpponentsFragment();
        ...
        if (!isInCommingCall){
            addConversationFragmentStartCall(opponentsList, conferencType, null);
        }
    }
    ...
    private void initQBRTCClient() {
        ...
        rtcClient.setCameraErrorHendler(new VideoCapturerAndroid.CameraErrorHandler() {
            ...
            public void onCameraError(final String s) {
                CallActivityTemp.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toaster.longToast(s);
                    }
            ...
            public void run() {
                if (show) {
                    ...
                    if (connectionView.getParent() == null) {
                        ((ViewGroup) CallActivityTemp.this.findViewById(R.id.fragment_container)).addView(connectionView);
                    }
                } else {
                    ((ViewGroup) CallActivityTemp.this.findViewById(R.id.fragment_container)).removeView(connectionView);
                }
            }
        });
    ...
    public void initCurrentSession(QBRTCSession sesion) {
        ...
        this.currentSession.addSessionCallbacksListener(CallActivityTemp.this);
        this.currentSession.addSignalingCallback(CallActivityTemp.this);
    }

    public void releaseCurrentSession() {
        Log.d(TAG, "Release current session");
        this.currentSession.removeSessionCallbacksListener(CallActivityTemp.this);
        this.currentSession.removeSignalingCallback(CallActivityTemp.this);
    ...
    public void onSessionStartClose(final QBRTCSession session) {
        session.removeSessionCallbacksListener(CallActivityTemp.this);
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ConversationFragment fragment = (ConversationFragment) getFragmentManager().findFragmentByTag(CONVERSATION_CALL_FRAGMENT);
