...
class LoginActivity : BaseActivity() {
        ...
        QBUsers.signIn(qbUser).performAsync(object : QBEntityCallback<QBUser> {
            ...
            override fun onError(e: QBResponseException?) {
                ...
                if (e?.httpStatusCode == UNAUTHORIZED) {
                    ...
                    e?.let {
                        showErrorSnackbar(R.string.splash_signin_error, e, View.OnClickListener {
                            signIn()
                        })
                    }
                }
    ...
    private fun signUp(qbUser: QBUser) {
        ...
        QBUsers.signUp(qbUser).performAsync(object : QBEntityCallback<QBUser> {
            ...
            override fun onError(e: QBResponseException?) {
                Log.d(TAG, "SignUp Error: " + e?.message)
                e?.let {
                    showErrorSnackbar(R.string.splash_signup_error, e, View.OnClickListener {
                        signIn()
                    })
                }
            }
