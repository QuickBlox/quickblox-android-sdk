...
object QbUsersDbManager {
    ...
    fun getAllUsers(): ArrayList<QBUser> {
        ...
        if (cursor.moveToFirst()) {
            ...
            do {
                if (cursor.getInt(userIdColIndex) == userId) {
                    ...
                    val tags = StringifyArrayList<String>()
                    tags.add(*cursor.getString(userTagColIndex).split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray())
                    qbUser.tags = tags
                    break
                }
            } while (cursor.moveToNext())
    ...
    fun getUsersByIds(usersIds: List<Int>): ArrayList<QBUser> {
        ...
        for (userId in usersIds) {
            if (getUserById(userId) != null) {
                qbUsers.add(this.getUserById(userId)!!)
            }
        }
        return qbUsers
    }
